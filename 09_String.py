####################################################################
# 문자열 (문자가 열거된 형태)
#  .문자들을 일렬로 나열시켜 순서에 맞게 index를 부여한 상태 
####################################################################
...
# 문자열의 index로 문자를 변경할 수없다.
svalue[1] = 'p'


####################################################################
# 불변 객체 (immuteable)
# 데이터를 새로 갱신하여 재 배치 하는 프로세스 보다 메모리 손실을 감안하더라도
...
# stringIO 모듈
# 불변객체인 str 의 메모리 손실을 효율적으로 동작할 수 있도록 해주는 모듈
...
print(svalue)[::-2]   # 마지막 index 부터 -2칸씩 들여서 표현 y.otp
...
svalue = "Python Programming"
if 'P' in svalue : 
    print("대문자 P는 포함되어 있습니다.")

if 'p' not in svalue :
    print("소문자 p 는 포함되어 있지 않습니다.")



# 삼항 연산자 (조건의 참 / 거짓의 로직을 한 줄로 표현 )
# [참 인 경우의 로직] if 조건 else [거짓인 경우 로직]
svalue = "Python Programming"
...
## startswith 시작 하는지 확인.
print('Python 관련 데이터 입니다'                # 참 
      if svalue.startswith("Python")             #조건
      else "Python 관련 데이터가 아닙니다")      # 거짓



## endswith 종료하는지 확인 (프로그램의 확장자 찾을 때 용이)
print('jpg 형식입니다.'                 # 참 
